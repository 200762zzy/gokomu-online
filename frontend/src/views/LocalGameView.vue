...
<template>
  ...
  <!-- MOBILE -->
  <template v-else>
    <div class="mobile-local-layout">
      <div class="mobile-local-board">
        <GameBoard
          :board="displayBoard"
          :current-turn="getStoneName(currentTurn)"
          :game-over="gameOver"
      ...
      <div v-if="!replayMode" class="mobile-local-info">
        <GameInfo
          :current-turn="getStoneName(currentTurn)"
          :game-over="gameOver"
          :winner="winner"
...
.mobile-local-info .action-row {
  display: flex;
  gap: 8px;
}

.mobile-local-info .action-row .btn-action {
  flex: 1;
  min-height: 44px;
}

.mobile-local-info .btn-action {
  min-height: 44px;
}
